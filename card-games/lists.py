...
def get_rounds(number: int) -> list:
    ...
    rounds: list = [number, number + 1, number + 2]
    return rounds
...
def approx_average_is_average(hand: list) -> bool:
    ...
    median: float = hand[(len(hand)// 2)]
    approx_value: float = (hand[0] + hand[-1]) / 2
    actual_approx: float = sum(hand) / len(hand)
    if median == actual_approx or approx_value == actual_approx:
        return True
    return False
